...
The same build can write `drafts/stories.json` with `--stories-json`. That file is the Boardwalk import: one record per canonical URL, with the feed summary and byline when the feed has them, and no article body. A link with a non-numeric port, a port other than the scheme default, a URL longer than 2,000 characters, or an IPv6 address is left out of the file. The timer pushes it only when pass entry `njpbs/boardwalk/around-nj-import` exists. A missing entry skips the push. An empty export, a token that is not 48 letters or digits, or a failed request fails the push after the HTML snapshot is written. The HTML snapshot and the newsletter do not change.
